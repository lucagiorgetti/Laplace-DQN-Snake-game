...
*On the left, DQN Loss function: even it is mostly flat and zero, the peaks show that RMSProp is still oprimizing the model; On the right, Projection of $D$ along its two first singular directions: while the second one seems to be periodic, the first one is not.*
